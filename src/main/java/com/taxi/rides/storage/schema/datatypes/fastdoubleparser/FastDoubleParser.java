...
/*
 * copied from https://github.com/wrandelshofer/FastDoubleParser
 */

/**
 * This is a C++ to Java port of Daniel Lemire's fast_double_parser.
 *
 * <p>The code has been changed, so that it parses the same syntax as {@link
...
public class FastDoubleParser {
   ...
   *
   * <p>However for some reason, performance is best, if this table has exactly 256 entries.
   */
  ...
  private static NumberFormatException newNumberFormatException(
    ...
    if (str.length() > 1024) {
      ...
      return new NumberFormatException(
          "For input string: \"" + str.subSequence(startIndex, endIndex) + "\"");
    }
  }
  ...
  /**
   * Returns a Double object holding the double value represented by the argument string {@code
   * str}.
   *
   * <p>This method can be used as a drop in for method {@link Double#valueOf(String)}. (Assuming
   * that the API of this method has not changed since Java SE 16).
   *
   * <p>Leading and trailing whitespace characters in {@code str} are ignored. Whitespace is removed
   * as if by the {@link String#trim()} method; that is, characters in the range [U+0000,U+0020].
   *
   * <p>The rest of {@code str} should constitute a FloatValue as described by the lexical syntax
   * rules shown below:
   *
   * <blockquote>
   *
   ...
   * @throws NumberFormatException if the string can not be parsed
   */
  public static double parseDouble(CharSequence str, int offset, int length)
  ...
  private static double parseInfinity(
      CharSequence str, int startIndex, int endIndex, boolean negative) {
  ...
  private static double parseRestOfDecimalFloatLiteral(
    ...
    double result =
        FastDoubleMath.decFloatLiteralToDouble(
            index,
            isNegative,
            digits,
            exponent,
    ...
    return Double.isNaN(result)
        ? parseRestOfDecimalFloatLiteralTheHardWay(str, startIndex, endIndex)
        : result;
  }
   ...
   */
  private static double parseRestOfDecimalFloatLiteralTheHardWay(
      CharSequence str, int startIndex, int endIndex) {
  ...
  private static double parseRestOfHexFloatingPointLiteral(
    ...
    double d =
        FastDoubleMath.hexFloatLiteralToDouble(
            index,
            isNegative,
            digits,
            exponent,
