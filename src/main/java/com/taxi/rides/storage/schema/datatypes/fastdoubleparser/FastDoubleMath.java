package com.taxi.rides.storage.schema.datatypes.fastdoubleparser;
 ...
 *
 * <p>This is a C++ to Java port of Daniel Lemire's fast_double_parser.
 *
 * <p>The code contains enhancements from Daniel Lemire's fast_float_parser, so that it can parse
 * double Strings with very long sequences of numbers
 *
 * <p>References:
 *
 * <dl>
 *   <dt>Daniel Lemire, fast_double_parser, 4x faster than strtod. Apache License 2.0 or Boost
 *       Software License.
 *   <dd><a href="https://github.com/lemire/fast_double_parser">github.com</a>
 *   <dt>Daniel Lemire, fast_float number parsing library: 4x faster than strtod. Apache License
 *       2.0.
 *   <dd><a href="https://github.com/fastfloat/fast_float">github.com</a>
 *   <dt>Daniel Lemire, Number Parsing at a Gigabyte per Second, Software: Practice and Experience
 *       51 (8), 2021. arXiv.2101.11408v3 [cs.DS] 24 Feb 2021
 *   <dd><a href="https://arxiv.org/pdf/2101.11408.pdf">arxiv.org</a>
 * </dl>
 */
class FastDoubleMath {
  /**
   * The smallest non-zero float (binary64) is 2^−1074. We take as input numbers of the form w x
   * 10^q where w < 2^64. We have that {@literal w * 10^-343 < 2^(64-344) 5^-343 < 2^-1076}.
   *
   * <p>However, we have that {@literal (2^64-1) * 10^-342 = (2^64-1) * 2^-342 * 5^-342 > 2^−1074}.
   * Thus it is possible for a number of the form w * 10^-342 where w is a 64-bit value to be a
   * non-zero floating-point number.
   *
   * <p>********
   *
   * <p>If we are solely interested in the *normal* numbers then the smallest value is 2^-1022. We
   * can generate a value larger than 2^-1022 with expressions of the form w * 10^-326. Thus we need
   * to pick FASTFLOAT_SMALLEST_POWER >= -326.
   *
   * <p>********
   *
   * <p>Any number of form w * 10^309 where w>= 1 is going to be infinite in binary64 so we never
  ...
  /**
   * Precomputed powers of ten from 10^0 to 10^22. These can be represented exactly using the double
   * type.
   */
  ...
  /**
   * When mapping numbers from decimal to binary, we go from w * 10^q to m * 2^p but we have 10^q =
   * 5^q * 2^q, so effectively we are trying to match w * 2^q * 5^q to m * 2^p. Thus the powers of
   * two are not a concern since they can be represented exactly using the binary notation, only the
   * powers of five affect the binary significand.
   *
   * <p>
   *
   * <p>The mantissas of powers of ten from -308 to 308, extended out to sixty four bits. The array
   ...
   * accessed.
   */
  private static final long[] MANTISSA_128 = {
  ...
  static double decFloatLiteralToDouble(
      ...
      if (FASTFLOAT_DEC_SMALLEST_POWER <= exponentOfTruncatedDigits
        ...
        double roundedUp =
            tryDecToDoubleWithFastAlgorithm(
                isNegative, digits + 1, (int) exponentOfTruncatedDigits);
    ...
    } else if (FASTFLOAT_DEC_SMALLEST_POWER <= exponent
        && exponent <= FASTFLOAT_DEC_LARGEST_POWER) {
   ...
   *
   * <dl>
   *   <dt>Getting the high part of 64 bit integer multiplication
   *   <dd><a
   *       href="https://stackoverflow.com/questions/28868367/getting-the-high-part-of-64-bit-integer-multiplication">
   *       stackoverflow</a>
   * </dl>
   *
  ...
  static double hexFloatLiteralToDouble(
    ...
    if (isDigitsTruncated) {
      ...
      // There are cases, in which rounding has no effect.
      if (FASTFLOAT_HEX_SMALLEST_POWER <= truncatedExponent
          && truncatedExponent <= FASTFLOAT_HEX_LARGEST_POWER) {
        double withoutRounding =
            tryHexToDoubleWithFastAlgorithm(isNegative, digits, (int) truncatedExponent);
        double roundedUp =
            tryHexToDoubleWithFastAlgorithm(isNegative, digits + 1, (int) truncatedExponent);
        if (!Double.isNaN(withoutRounding) && Objects.equals(roundedUp, withoutRounding)) {
          return withoutRounding;
        }
      }
    ...
    } else if (FASTFLOAT_HEX_SMALLEST_POWER <= exponent
        && exponent <= FASTFLOAT_HEX_LARGEST_POWER) {
  ...
  /**
   * Attempts to compute {@literal digits * 10^(power)} exactly; and if "negative" is true, negate
   * the result.
   *
   * <p>This function will only work in some cases, when it does not work it returns null. This
   * should work *most of the time* (like 99% of the time). We assume that power is in the
   * [FASTFLOAT_SMALLEST_POWER, FASTFLOAT_LARGEST_POWER] interval: the caller is responsible for
   * this check.
   *
  ...
  static double tryDecToDoubleWithFastAlgorithm(boolean isNegative, long digits, int power) {
    ...
    if ((upper & 0x1FF) == 0x1FF && Long.compareUnsigned(lower + digits, lower) < 0) {
      ...
      if (((product_middle + 1 == 0)
          ...
          && (product_low + Long.compareUnsigned(digits, product_low)
              < 0))) { // let us be prudent and bail out.
        return Double.NaN;
      }
      upper = product_high;
  ...
  /**
   * Attempts to compute {@literal digits * 2^(power)} exactly; and if "negative" is true, negate
   * the result.
   *
   * <p>This function will only work in some cases, when it does not work it returns null.
   *
   * @param isNegative whether the number is negative
   * @param digits uint64 the digits of the number
   * @param power int32 the exponent of the number
